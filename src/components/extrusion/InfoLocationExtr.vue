    ...
    <form>
      <b-container fluid class="p-0">
        ...
        <b-row>
          <b-col cols="9" >
            ...
            <label for="location" class="col-sm-2 col-form-label sr-only">Location</label>
              <b-form-input
                ...
                v-model="location"
                :placeholder='$t("infolocationextr.scanlocation")'
          ...
          <!-- <b-col cols="4">
             <b-form-select v-model="selected" :options="options2" @change="executesql" v-b-tooltip.hover.top="'Select type'"></b-form-select>
          </b-col> -->
          <b-col cols="3">
            <b-button variant="primary" v-b-tooltip.hover :title="ps_printid"
              type="submit" block
              id="btnSearch"
              ref="btnSearch"
...
export default {
  ...
  watch: {
    location: function (v) {
      ...
      this.level = ''
      if (this.location.slice(-1) === 'L' || this.location.slice(-1) === 'R' || this.location.slice(-1) === '-') {
        this.showlevel = true
      } else {
  ...
  methods: {
    ...
    onChange: function () {
      ...
      // console.log(this.sellabelprinter)
      // console.log(this.sellabeltype)
    },
    async getprinters () {
      await axios
        .get(
    ...
    executesql () {
      ...
      if (this.loggedinuser !== '' && this.loginSuccessful === true) {
        if (this.location !== '') {
          ...
          this.feedback = this.$t('general.busy') + ' ' + this.location + this.level
          NProgress.start()
          axios
            .get(
            ...
            .then(res => {
              ...
              if (!resultArray.length) {
                ...
                this.feedback = 'No data ' + this.location + this.level
                this.$refs.location.focus()
    ...
    print () {
      ...
      if (this.ps_printid !== 0) {
        ...
        this.feedback = this.$t('infolocationextr.noprintid')
        this.printvisible = false
        this.container = ''
