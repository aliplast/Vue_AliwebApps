      ...
      <b-container fluid class="p-0">
        <b-row>
          <b-col cols="12">
            <div v-bind:class="getClass()">
              <h2>
                {{ $t("transpreg.title") }} {{customercode}}
                <h6>{{feedback}}</h6>
              </h2>
            </div>
          </b-col>
        ...
        <b-row>
          <b-col cols="9">
            <!-- Search Customer -->
            <b-form-input
              :placeholder="this.$t('transpreg.searchcustomer')"
              ref="customer"
              id="customer"
              type="text"
              autocomplete="off"
          ...
          <b-col cols="3">
            <b-button v-b-tooltip.hover  :title="this.$t('transpreg.in')" block
              :pressed.sync="directionout"
              v-if="!directionout"
              variant="outline-secondary"
            ><b-icon icon="box-arrow-in-down"></b-icon></b-button>
            <b-button  v-b-tooltip.hover  :title="this.$t('transpreg.out')" block
              :pressed.sync="directionout"
        ...
        <b-row>
          <b-col cols="6">
            ...
            <b-form-input
              ...
              type="text"
              v-model="container"
              :placeholder="this.$t('transpreg.scancontainer')"
...
export default {
  ...
  watch: {
    ...
    customer () {
      if (this.customer.length > 0) { this.filterCustomers() }
      this.rows = []
    }
  },
  mounted () {
    ...
    add2table () {
      ...
      if (!this.exists) {
        if (this.container.match('^[0-9]{1,8}$')) {
          ...
          this.feedback = this.$t('general.real') + ' ' + this.container
          ...
          this.feedback = this.$t('general.container') + ' ' + this.container
        } else {
          // this.type = ''
          this.remark = ''
          this.feedback = this.$t('transpreg.invalidcontainer')
          this.errorfound = true
        }
      } else {
        this.feedback = this.$t('transpreg.duplicatecontainer')
        this.errorfound = true
        ...
        if (this.customer !== '' && this.rows.length > 0) {
          ...
          axios
            ...
            .then(res => {
              ...
              this.feedback = this.$t('general.completed')
